...
function renderBlockVideo(meta) {  
  const videoId = getId(String(meta.source));
  const videoSrc = 'https://www.youtube.com/embed/' + videoId
  return (
    <div className="nso-video">
...
const NotionBlockRenderer = ({ data, renderer, debug }) => {
  ...
  return (
    <div>      
      <div>
        <h1 className="title nso-title has-text-primary is-size-1">{notionPageBlog.title}</h1>
      </div>
      <div>{child}</div>
      {debug && (
        <div>
